  ...
  describe("events", function() {
      ...
      describe("when the event is valid", function() {
        ...
        describe("when the message is for a hook event", function() {
          ...
          it("calls the handler and sends back the response", function() {
            var retval = window.addEventListener.lastCall.args[1].call(
              subject,
              evt
            );
            ...
            it("calls the handler and sends back the rejection value as an error", function() {
              ...
              return retval.then(function() {
                ...
                expect(source.postMessage).to.have.been.calledWith(
                  {
                    appGuid: "ABC123",
                    error: { msg: "The third party API is broken." },
                    key: "iframe.reply:hello"
                  },
                  "https://foo.zendesk.com"
                );
    ...
    describe("#postMessage", function() {
      ...
      it("waits until the client is ready to post messages", function() {
        ...
        expect(source.postMessage).to.not.have.been.calledWithMatch({
          key: "foo",
          appGuid: "ABC123",
          instanceGuid: "ABC123"
        });
      ...
      it("posts a message when the client is ready", function() {
        subject.ready = true;
        subject.postMessage("foo");
        expect(source.postMessage).to.have.been.calledWithMatch({
          key: "foo",
          appGuid: "ABC123",
          instanceGuid: "ABC123"
        });
    ...
    describe("#on", function() {
      ...
      it("notifies the framework of the handler registration", function() {
        ...
        expect(
          subject.postMessage
        ).to.have.been.calledWithMatch("iframe.on:foo", { subscriberCount: 1 });
      });
    });
    ...
    describe("#off", function() {
      ...
      it("notifies the framework of the handler removal", function() {
        ...
        expect(
          subject.postMessage
        ).to.have.been.calledWithMatch("iframe.off:foo", {
          subscriberCount: 0
        });
      });
    ...
    describe("#trigger", function() {
      ...
      it("posts a message so the framework can trigger the event on all registered clients", function() {
        ...
        expect(subject.postMessage).to.have.been.calledWith(
          "iframe.trigger:foo",
          data
        );
      });
    ...
    describe("#request", function() {
      ...
      it("asks ZAF to make a request", function() {
        expect(
          subject.postMessage
        ).to.have.been.calledWithMatch(/request:\d+/, {
      ...
      describe("promise", function() {
        ...
        it("resolves when the request succeeds", function(done) {
          ...
          promise.then(function() {
            expect(doneHandler).to.have.been.calledWith(
        ...
        it("rejects when the request fails", function(done) {
          ...
          promise.then(function() {
            expect(failHandler).to.have.been.calledWith(
              response.responseArgs[0]
            );
            done();
          });
  ...
  describe("v2 methods", function() {
    ...
    describe("#get", function() {
      it("takes an argument and returns a promise with data", function(done) {
        ...
        expect(promise).to.eventually
          .become({ errors: {}, "ticket.subject": "test" })
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: { errors: {}, "ticket.subject": "test" }
          }
        });
      });
      ...
      it("throws an error when the handler throws it", function(done) {
        ...
        expect(promise).to.be
          .rejectedWith(Error, "ticket.err unavailable")
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: {
              errors: { "ticket.err": { message: "ticket.err unavailable" } }
            }
          }
        });
      });
      ...
      it("resolves with errors when bulk requesting", function(done) {
        ...
        expect(promise).to
          .become({ errors: { "ticket.subj": { message: "No such Api" } } })
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: { errors: { "ticket.subj": { message: "No such Api" } } }
          }
        });
      });
      ...
      it("rejects the promise after 5 seconds", function(done) {
        ...
        clock.restore();
        expect(promise).to.be
          .rejectedWith(Error, "Invocation request timeout")
          .and.notify(done);
      });
    ...
    describe("#set", function() {
      it("takes two arguments and returns a promise with data", function(done) {
        ...
        expect(promise).to.eventually
          .become({ errors: {}, "ticket.subject": "value" })
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: { errors: {}, "ticket.subject": "value" }
          }
        });
      });
      ...
      it("rejects the promise when single request and handler throws an error", function(
        done
      ) {
        promise = subject.set("ticket.foo", "bar");

        expect(promise).to.be
          .rejectedWith(Error, "ticket.foo unavailable")
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: {
              errors: { "ticket.foo": { message: "ticket.foo unavailable" } }
            }
          }
        });
      });
      ...
      it("resolves with errors when bulk requesting", function(done) {
        ...
        expect(promise).to
          .become({ errors: { "ticket.foo": { message: "No such Api" } } })
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: { errors: { "ticket.foo": { message: "No such Api" } } }
          }
        });
      });
    ...
    describe("#invoke", function() {
      it("takes multiple arguments and returns a promise with data", function(
        done
      ) {
        ...
        expect(promise).to.eventually
          .become({ errors: {}, "ticket.appendText": true })
          .and.notify(done);
        ...
        window.addEventListener.callArgWith(1, {
          ...
          data: {
            id: requestsCount,
            result: { errors: {}, "ticket.appendText": true }
          }
        });
      });
      ...
      it("doesnt reject whitelisted promises after 5 seconds", function(done) {
        ...
        clock.restore();
        expect(promise).to.eventually
          .become({ errors: {}, "instances.create": { url: "http://a.b" } })
          .and.notify(done);
        window.addEventListener.callArgWith(1, {
          origin: subject._origin,
          source: subject._source,
          data: {
            id: requestsCount,
            result: { errors: {}, "instances.create": { url: "http://a.b" } }
          }
        });
      });
    });
    ...
    describe("#context", function() {
      ...
      it("waits for the app to be registered before resolving", function() {
        promise = subject.context();
        triggerEvent(subject, "app.registered", {
          metadata: {},
          context: context
        });
        return expect(promise).to.eventually.eq(context);
    ...
    describe("#instance", function() {
      ...
      it("returns a client for the instance", function() {
        ...
        expect(instanceClient).to.have
          .property("_instanceGuid")
          .that.equals("def-321");
      });

      it("returns the same client when requested multiple times", function() {
        expect(subject.instance("def-321")).to.equal(
      ...
      describe("with the returned client", function() {
        ...
        describe("#context", function() {
          ...
          it("delegates to instances api", function() {
            sandbox
              .stub(childClient, "get")
              .withArgs("instances.def-321")
              .returns(Promise.resolve({ "instances.def-321": context }));
            promise = childClient.context();
            expect(childClient.get).to.have.been.calledWith(
              "instances.def-321"
            );
