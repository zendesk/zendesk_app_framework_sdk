...
function throwUnlessBulk(path, result) {
  ...
  if(isSingularOperation && result.errors && result.errors[path]) {
    var err = new Error(result.errors[path].message);
    err.name = result.errors[path].name;
    err.stack = result.errors[path].stack;
    throw err;
  }
