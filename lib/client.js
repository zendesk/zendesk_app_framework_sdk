...
Client.prototype = {
  ...
  /// var client = ZAFClient.init();
  ///
  /// client.get(['ticket.subject', 'ticket.fields.custom_field_1234.value']).then(function(data) {
  ///   console.log(data); // ['Help, my printer is on fire', 'Printer on Fire']
  /// });
  /// ```
  get: function(paths) {
    var wasArray = Array.isArray(paths);
    if (!wasArray) {
      paths = [ paths ];
    }
    var promise = wrappedPostMessage.call(this, 'data.get', paths);
    return (wasArray) ? promise : promise.then(function(data) {
      return paths[0].split('.').reduce(function(memo, pathSegment) {
        return memo[pathSegment];
      }, data);
    });
  },
  ...
  /// var client = ZAFClient.init();
  ///
  /// client.set('ticket', { subject: 'Printer Overheating Incident', type: 'incident' }).then(function(ticket) {
  ///   console.log(ticket);
  /// });
  /// ```
  set: function(key, val) {
    ...
    if ('string' === typeof key) {
      // namespace the setted object
      obj = key.split('.').reverse().reduce(function(memo, pathSegment) {
        var ret = {};
        ret[pathSegment] = memo;
        return ret;
      }, val);

      return wrappedPostMessage.call(this, 'data.set', obj).then(function(ret) {
        // denamespace the return value
        return key.split('.').reduce(function(memo, pathSegment) {
          return memo[pathSegment];
        }, ret);
      });
    } else {
      return wrappedPostMessage.call(this, 'data.set', obj);
    }
  },
  ...
  /// var client = ZAFClient.init();
  ///
  /// client.invoke('modal.show', { url: 'https://www.example.com/modal.html' }).then(function(data) {
  ///   console.log(data); // { width: 200, height: 200 }
  /// });
  /// ```
  invoke: function() {
    wrappedPostMessage.bind(this, 'call').apply(null, arguments);
  }
};
