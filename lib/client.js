...
const PROMISE_TIMEOUT = 5000 // 5 seconds
...
function timeoutReject (reject, name, paramsArray) {
  switch (name) {
    case 'invoke': {
      const matches = paramsArray.filter(function (action) {
...
export default class Client {
  ...
  get (path) {
    ...
    if (arguments.length > 1 || paths.some(function (path) { return !isString(path) })) {
  ...
  invoke (keyOrObject) {
    ...
    if (typeof keyOrObject === 'string') {
      ...
      Object.keys(keyOrObject).forEach(function (key) {
        ...
        if (!Array.isArray(methodArgs) || methodArgs.some(function (arg) { return !isString(arg) })) {
