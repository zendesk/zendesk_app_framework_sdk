...
function createError(error) {
  if (error.path) { error.msg = '"'error.path + '" ' + error.msg; }
...
function throwUnlessBulk(path, result) {
  ...
  if(isSingularOperation && result.errors) {
    var err = result.errors.find(function(error) { return error.path === path; });
...
Client.prototype = {
  ...
  ///
  /// Create an Client wrapper for another app instance.
  ///
  /// ##### Arguments
  ///
  ///   * `instanceGuid` the GUID of the other instance of your app
  ///
  /// ##### Returns
  ///
  /// A [Promises/A+](https://promisesaplus.com) conformant `promise` object.
  ...
  ///   var guid = Object.keys(data.instances).filter(function(guid) {
  ///     return data.instances[guid].location == 'top_bar';
  ///   })[0];
  ///   return guid;
  /// }).then(function(guid) {
  ///   return client.instance(guid);
  /// }).then(function(topBarClient) {
  ...
  get: function(path) {
    ...
    return wrappedPostMessage.call(this, 'get', paths).then(throwUnlessBulk.bind(null, path));
  },
  ...
  set: function(key, val) {
    ...
    return wrappedPostMessage.call(this, 'set', obj).then(throwUnlessBulk.bind(null, key));
  },
  ...
  invoke: function(key) {
    ...
    return wrappedPostMessage.call(this, 'invoke', obj).then(throwUnlessBulk.bind(null, key));
  }
};
