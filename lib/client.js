...
/* params is an array for set/invoke or an object for set */
...
Client.prototype = {
  ...
  get: function(path) {
    var paths = Array.isArray(path) ? path : Array.prototype.slice.call(arguments, 0);

    if (paths.filter(function(s) { return typeof s !== 'string'; }).length) { return Promise.reject('The get method accepts a string or array of strings.'); }
  ...
  set: function(key, val) {
    ...
    if (typeof key === 'string') {
      if (arguments.length === 1) { return Promise.rejectect('The setter requires a value'); }
      obj = {};
      obj[key] = val;
    }

    if (obj.toString() !== '[object Object]') { Promise.reject('The set method accepts 2 string, or an object.'); }
  ...
  invoke: function() {
    ...
    return wrappedPostMessage.call(this, 'invoke', params);
  }
};
