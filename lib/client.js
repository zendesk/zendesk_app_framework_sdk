...
export default class Client {
  constructor (options) {
    ...
    // If the host reaches out to the app via ZAF, it will make something like a 'poll.getIdleState' request.
    // We should respond with the current state of the iframe.
    this.on('poll.getIdleState', (data) => {
      // send a postMessage responding with the current state
      const currentState = this._idleState.state
      this.postMessage('poll.getIdleState.response', { idleState: currentState })
    }, this)
