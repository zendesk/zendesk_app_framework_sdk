...
Client.prototype = {
  ...
  on: function(name, handler, context) {
    if (typeof handler == 'function') {
      var id = nextIdFor('event');
      
      this._messageHandlers[name] = this._messageHandlers[name] || [];
      this._messageHandlers[name].push(handler.bind(context || null));

      if (name !== 'app.registered') {
        this.postMessage('iframe.on:' + name, { id: id });
      }
    }
  },
  ...
  trigger: function(name, data) {
    if (!this._messageHandlers[name] || this._repliesPending[name]) { return false; }
    if (data && data.reply) {
      this._repliesPending[name] = true;
      when(this._messageHandlers[name]).then(
        this.postMessage.bind(this, 'iframe.reply:' + name)
      ).catch(function(reason) {
        this.postMessage('iframe.reply:' + name, { error: true, reason: reason }); 
      }.bind(this)).then(
        function() { delete this._repliesPending[name]; }
      );
    } else {
