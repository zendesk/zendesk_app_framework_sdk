...
Client.prototype = {
  ...
  /// ##### Arguments
  ///
  ///   * `name` the name of the message event. This will determine the name of the iframe
  ...
  ///     receives, apart from DOM events. You don't need to register these events on app first.
  ...
  ///   * `options` (optional) an options object that's compatible with `jQuery.ajax`
  ///   * `callback` a callback that will get called when the request completes
  ///
  /// ```javascript
  /// var client = ZAFClient.init();
  ///
  /// app.request('/api/v2/tickets.json', function(tickets) {
  ///   console.log(tickets);
  /// });
  /// ```
  request: function(url, options, callback) {
    ...
    this.on('request:' + requestId + '.always', function(evt) {
      callback.apply(this, evt.responseArgs);
    });

    this.postMessage('request:' + requestId, options);
  }
};
