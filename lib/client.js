...
export default class Client {
  constructor (options) {
    ...
    this._idleState = new IdleState(this._iframe_session_timeout)
    this._removeObserverCallback = null
    this.ready = false
    ...
    this.on('app.registered', (data) => {
      ...
      this._removeObserverCallback = this._idleState.addObserver(state => {
        if (state === 'active') {
    ...
    // If the host reaches out to the app via ZAF, it will make something like a 'get.IdleState' request.
    // This is required in order to determine the current value of the idleState of the app, so that ZAF can 
    // keep track of it on behalf of the host.
    this.on('get.IdleState', (data) => {
      // send a postMessage back to ZAF responding with the current state
      const currentState = this._idleState.state
      this.postMessage('get.IdleState.response', { idleState: currentState })
    }, this)
