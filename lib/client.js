...
function messageHandler(client, event) {
  ...
  var observable;
  if (data.id && (observable = observables[data.id])) {
    switch (data.event) {
      case 'onError':
        observable.onError(data.result);
        delete observables[data.id];
        break;
      case 'onCompleted':
...
Client.prototype = {
  ...
  /// var client = ZAFClient.init();
  ///
  /// client.ui('get', { path: ['ticket.customField.value'] }).subscribe(function(data) {
  ...
  ui: function(name, data) {
    ...
    return Observable.create(function(observer) {
      ...
      // save for later
      observables[id] = observer;

      // timeout the promise to ensure it will be garbage collected if nobody responds
      setTimeout(function() {
        observer.onError(new Error('UI Request Timeout'));
      }, PROMISE_TIMEOUT);

      var msg = JSON.stringify({ request: name, message: data, appGuid: client._appGuid, id: id });
